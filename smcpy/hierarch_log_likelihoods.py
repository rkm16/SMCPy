...
class ApproxHierarch(BaseLogLike):
    ...
    def __init__(self, model, data, args):
        ...
        :params args:  args[0] is the rand eff marginal log likelihoods and
                       args[1] is the prior probabilities of the posterior
    ...
    def __call__(self, inputs):
        ...
        for i, d in enumerate(self._data):
            ...
            mll = self._args[0][i]
            log_like[:, i] = mll + self._logsum(log_conditionals - log_priors)
...
class MVNHierarchModel:
    ...
    def __call__(self, data):
        cov = self._build_cov_array()
        r, c = self._inputs.shape
        term1 = np.full((r, 1), c * np.log(2 * np.pi))
        term2 = np.linalg.slogdet(cov)[1].reshape(-1, 1)
        term3 = self._compute_squared_mahalanobis(cov, data)
        return -1/2 * (term1 + term2 + term3)

    def _build_cov_array(self):
    ...
    def _compute_squared_mahalanobis(self, cov, data):
        ...
        return np.einsum('inj,ijk,ink->in', delta, VI, delta)
